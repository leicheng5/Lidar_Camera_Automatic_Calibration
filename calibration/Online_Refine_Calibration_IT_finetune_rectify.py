...
##############################################################################
def remove_large_outliers_std(errors, m=3.0):
    """
    Remove reprojection errors above mean + m * std.

    Any value greater than (mean + m·std) is considered an outlier and removed.

    Parameters:
        errors : np.ndarray of shape (N,) or (N,1)
        m      : multiplier for standard deviation (default 3.0)
                 - m=1.0   → keep ~68.2% of data (±1σ covers ~68.2%)
                 - m=1.5   → keep ~86.6% of data (±1.5σ covers ~86.6%)
                 - m=2.0   → keep ~95.4% of data (±2σ covers ~95.4%)
                 - m=3.0   → keep ~99.7% of data (±3σ covers ~99.7%)
                 - m=4.0   → keep ~99.99% of data (±4σ covers ~99.99%)

    Returns:
        filtered_errors : 1D np.ndarray of all errors ≤ threshold
        threshold       : float, cutoff value = mean + m·std
        mask            : 1D boolean array of length N (True = kept, False = outlier)
    """
    # Flatten to 1D
    e = errors.ravel()
    ...
    # Build mask of non-outliers
    mask = e <= threshold

    # Filtered errors
    filtered_errors = e[mask]
...
def Get_reproject_err(lidar_points, img_points, homography_matrix, max_dis_cost=150):
    ...
    reprojection_errors = reprojection_errors[reprojection_errors <= max_dis_cost]
    # reprojection_errors = remove_large_outliers_std(reprojection_errors, m=1.0)
...
def filter_img_points(img_coords, width=1920, height=1080):
    """

    Parameters:
        img_coords: np.ndarray of shape (N,  2)
        width:       image width (default 1920)
        height:      image height (default 1080)

    Returns:
        filtered:   np.ndarray of shape (M,  2), M ≤ N
    """
...
if __name__ == "__main__":
    ...
    cam_frame_dir   = r'/xdisk/caos/leicheng/FHWA/lei_camera_dataset4_rectify'
    lid_frame_dir   = r'/xdisk/caos/leicheng/FHWA/lei_lidar_dataset4_rectify'
    classes_path    = '../coco_classes.txt' #os.path.join(parent_dir, 'coco_classes.txt')
    train_data_file = '/xdisk/caos/leicheng/FHWA/train_Dataset4_paths.txt'
    #----------------------------------------------------------------------------------------------------------------------------#
    ...
    max_dis_cost    =  150
    #------------------------------------------------------#
    output_path = './calib_results_rectify'
    if not os.path.exists(output_path):
        os.makedirs(output_path)   
    save_path = './video_results_rectify'
    ...
    #for i in tqdm(range(101)):
        ...
        img_points   = filter_img_points(img_points, width=1920, height=1080)
        ...
        if len(point_pairs) % Frames_GAP == 0  or len(point_pairs) == len(cam_timestps): #total_length=len(cam_timestps)
            ...
            point_pairs_grid = select_points_with_grid(point_pair_100, image_width = 1920, image_height = 1080, 
                                                       num_cols = 48, num_rows = 27, interval=1)
