...
from utils_finetune.utils import resize_image, get_classes
...
def Compute_reproject_err(lidar_points, img_points, homography_matrix, width=1440, height=1080, max_dis_cost=150):
    ...
    return reprojection_error, rmse, lid_cent_proj, lid_cent, img_cent
...
def Get_reproject_err(lidar_points, img_points, homography_matrix):
    ...
    return reprojection_error, rmse, lid_cent_proj.squeeze(1)
...
def plot_paired_points(img_fr, img_points, radar_points, i, path, matrix, mode='homography'):
    ...
    return np.squeeze(transformed_points, axis=1)
    
    
def plot_points_only(img_fr, img_points, transformed_points, i, path):
    # Create a new figure for each plot
    plt.figure(dpi=300)
    ...
    # Plot the Projected Points
    if transformed_points.size>0:
        plt.scatter(transformed_points[:, 0], transformed_points[:, 1], s=4, c='green', label='Lidar')
...
if __name__ == "__main__":
    ...
    save_path2 = './DL_video_results_all_points'
    ...
    model = FineTune_Model(pretrain_path = None, input_shape = input_shape, out_dim = out_dim, token_len = token_len)
    ...
    #for i in tqdm(range(1)):
        ...
        rep_err_org, rmse_org, proj_lid_cents, new_lid_points, new_img_points = Compute_reproject_err(lidar_points, img_points, homography_matrix, width=img_fr.shape[1], height=img_fr.shape[0], max_dis_cost=150)
        
        #----------------------#
        #   Forward Propagation
        #----------------------#
        img_fr_chw  = resize_image(image, input_shape, letterbox_image=False)
        img_fr_chw  = np.transpose(img_fr_chw, (2, 0, 1))  # Channels first
        # Add a batch dimension to make it [batch_size, channels, height, width]
        img_fr_bchw = np.expand_dims(img_fr_chw, axis=0)  # Add batch dimension
        if Cuda:
            images         = torch.from_numpy(img_fr_bchw).type(torch.FloatTensor).to('cuda')
            lidars         = torch.from_numpy(lidar).to('cuda')
            img_cents      = [torch.from_numpy(new_img_points).to(dtype=torch.float32).to('cuda')]
            lid_cents      = [torch.from_numpy(new_lid_points).to(dtype=torch.float32).to('cuda')]
            lid_cents_proj = [torch.from_numpy(proj_lid_cents).to(dtype=torch.float32).to('cuda')]   
        delta_matrix, _ = model(images, lidars, img_cents, lid_cents, lid_cents_proj)
        delta_matrix    = delta_matrix.cpu().detach().numpy()
        delta_matrix    = delta_matrix.reshape(-1, 3, 3).squeeze()
        if not Intrinsic_Learning:
            ### Delta            
            new_matrix = homography_matrix + delta_matrix           
        else:
            ### Intrinsic
            intrinsic_matrix = delta_matrix
            new_matrix = np.matmul(homography_matrix, intrinsic_matrix)  # Shape: [B, 3, 3]
            
        rep_err_new, rmse_new, new2_proj_lid_points = Get_reproject_err(new_lid_points, new_img_points, new_matrix)
        ...
        if rep_err_org > rep_err_new:
            ...
            all_img_ctds.append(new_img_points)
            all_lid_ctds.append(new2_proj_lid_points)
        else:
            rep_err += rep_err_org
            rmse    += rmse_org
            matrix = homography_matrix
            all_img_ctds.append(new_img_points)
            all_lid_ctds.append(proj_lid_cents)
        projected_lids = plot_paired_points(img_fr, img_points, lidar_points, i, save_path, matrix)
        
        num_fr = i+1
        
        ##### Plot all points
        # all_img_ctds.append(img_points)
        # all_lid_ctds.append(projected_lids)
        all_img_points = np.concatenate(all_img_ctds, axis=0)
        all_lidar_points = np.concatenate(all_lid_ctds, axis=0)
        plot_points_only(img_fr, all_img_points, all_lidar_points, i, save_path2)
